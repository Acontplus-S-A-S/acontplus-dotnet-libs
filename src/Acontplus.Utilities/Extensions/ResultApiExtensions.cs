...
public static class ResultApiExtensions
{
    ...
    // Minimal API: GET: 200 OK or 204 NoContent
    public static IResult ToGetMinimalApiResult<T>(this Result<T, DomainErrors> result)
    {
        return result.Match<IResult>(
            value => value is null ? TypedResults.NoContent() : TypedResults.Ok(value),
            errors => TypedResults.Problem(errors.ToString())
        );
    }

    // Minimal API: POST: 201 Created (with Location header)
    public static IResult ToCreatedMinimalApiResult<T>(this Result<T, DomainErrors> result, string locationUri)
    {
        return result.Match<IResult>(
            value => TypedResults.Created(locationUri, value),
            errors => TypedResults.Problem(errors.ToString())
    ...
    // Minimal API: PUT: 200 OK or 204 NoContent
    public static IResult ToPutMinimalApiResult<T>(this Result<T, DomainErrors> result)
    {
        return result.Match<IResult>(
            value => value is null ? TypedResults.NoContent() : TypedResults.Ok(value),
            errors => TypedResults.Problem(errors.ToString())
    ...
    // Minimal API: DELETE: 204 NoContent or 404 NotFound
    public static IResult ToDeleteMinimalApiResult(this Result<bool, DomainErrors> result)
    {
        return result.Match<IResult>(
            deleted => deleted ? TypedResults.NoContent() : TypedResults.NotFound(),
            errors => TypedResults.Problem(errors.ToString())
    ...
    // Minimal API: GET: 200 OK or 204 NoContent (async)
    public static async Task<IResult> ToGetMinimalApiResultAsync<T>(this Task<Result<T, DomainErrors>> resultTask)
    {
        var result = await resultTask;
        return result.ToGetMinimalApiResult();
    }
    ...
    // Minimal API: POST: 201 Created (async)
    public static async Task<IResult> ToCreatedMinimalApiResultAsync<T>(this Task<Result<T, DomainErrors>> resultTask, string locationUri)
    {
        var result = await resultTask;
        return result.ToCreatedMinimalApiResult(locationUri);
    }
    ...
    // Minimal API: PUT: 200 OK or 204 NoContent (async)
    public static async Task<IResult> ToPutMinimalApiResultAsync<T>(this Task<Result<T, DomainErrors>> resultTask)
    {
        var result = await resultTask;
        return result.ToPutMinimalApiResult();
    }
    ...
    // Minimal API: DELETE: 204 NoContent or 404 NotFound (async)
    public static async Task<IResult> ToDeleteMinimalApiResultAsync(this Task<Result<bool, DomainErrors>> resultTask)
    {
        var result = await resultTask;
        return result.ToDeleteMinimalApiResult();
    }
