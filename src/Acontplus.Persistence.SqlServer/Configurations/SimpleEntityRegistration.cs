...
public static class SimpleEntityRegistration
{
    ...
    private static Type GetPrimaryKeyType(Type entityType)
    {
        ...
        return typeof(int); // or typeof(Guid), depending on your default
    }
        ...
        params Type[] entityTypes)
    {
        ...
        {
            // Check if entity inherits from AuditableEntity<> (more flexible check)
            ...
            try
            {
                ...
                // Try different possible generic type configurations
                Type baseConfigurationType;
                object baseConfiguration;

                // Option 1: BaseEntityTypeConfiguration<TEntity, TKey>
                try
                {
                    baseConfigurationType = typeof(BaseEntityTypeConfiguration<,>).MakeGenericType(entityType, keyType);
                    baseConfiguration = Activator.CreateInstance(baseConfigurationType);
                    modelBuilder.ApplyConfiguration((dynamic)baseConfiguration);
                }
                catch (ArgumentException)
                {
                    // Option 2: BaseEntityTypeConfiguration<TEntity> (single generic parameter)
                    try
                    {
                        baseConfigurationType = typeof(BaseEntityTypeConfiguration<,>).MakeGenericType(entityType);
                        baseConfiguration = Activator.CreateInstance(baseConfigurationType);
                        modelBuilder.ApplyConfiguration((dynamic)baseConfiguration);
                    }
                    catch (Exception ex)
