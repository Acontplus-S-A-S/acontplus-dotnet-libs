...
namespace Acontplus.Billing.Services.Validation;
...
public class RucService(IServiceProvider serviceProvider) : IRucService
{
    ...
    private async Task<Result<ContribuyenteCompleteDto, DomainError>> GetRucSriAsync(string idCard,
        ...
        string captcha)
    {
        var captchaDeserialized = captcha.DeserializeOptimized<TokenSri>();
        ...
        var rucs = sriResponse.DeserializeOptimized<List<ContribuyenteRucDto>>();
        ...
        string tokenSri)
    {
        ...
        var establecimientos = serializedEstabs.DeserializeOptimized<List<EstablecimientoDto>>();
